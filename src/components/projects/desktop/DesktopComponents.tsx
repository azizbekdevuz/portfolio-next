...
export const EditorTab = memo(
  ({
    ...
    >
      <span
        className={`w-2 h-2 rounded-full ${isActive ? "bg-primary" : "bg-primary/50"}`}
      />
      {project.title}.tsx
      {isActive && (
        <motion.div
...
export const TechBadge = memo(
  ({
    ...
    return (
      <div className="group flex items-center gap-2 rounded-lg border border-border/70 bg-card-muted/40 px-2.5 py-1.5 text-sm text-fg transition-colors duration-200 hover:border-border-strong hover:bg-card-muted/70">
