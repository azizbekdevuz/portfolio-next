...
function techCategoryOrder(track: ProofTrackFilter): string[] {
  ...
  if (track === "frontend")
    return ["frontend", "languages", "backend", "tools"];
  if (track === "fullstack")
    return ["backend", "frontend", "languages", "tools"];
...
export function ProofWorkspaceSurface({
  ...
  const lensTrackLabel =
    track === "all"
      ? messages.cockpit.allProof
      : messages.roleTracks[track].label;
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-3">
      ...
      >
        ...
        <p className="mt-1 text-[10px] leading-snug text-muted md:text-[11px]">
          {lensSummaryLine}
        </p>
      </div>
      <div
              ...
              <div className="space-y-4 p-4 md:p-5">
                ...
                <ul className="space-y-3">
                  {journeyData.slice(0, 4).map((j) => (
                    <li
                      key={j.id ?? j.title}
                      className="border-l-2 border-accent/35 pl-3"
                    >
              ...
              <div className="space-y-4 p-4 md:p-5">
                ...
                <ul className="space-y-3">
                  {flatAchievements.slice(0, 4).map((it, i) => (
                    <li
                      key={`${it.title}-${i}`}
                      className="rounded-lg border border-border bg-page-elevated/60 p-3"
                    >
